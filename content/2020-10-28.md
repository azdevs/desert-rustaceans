...
Rust's safety is attractive.  Marcus described how tooling in C++ provides safety but doesn't optimize code.  Venturing into optimization-space is where it gets dangerous.  There are tools like static analysis, but for large enough codebases (e.g. games) it doesn't catch everything.
...
Jacob notes that the best pattern for game development in Rust is [ECS](https://en.wikipedia.org/wiki/Entity_component_system) which is going to runtime check all your stuff anyway, so you are not getting as much bang for the buck.  An [excellent talk by Catherine West](https://www.youtube.com/watch?v=aKLntZcp27M) covers this in detail.
...
- Blaine took the unperformant client and optimized it by ripping out the tutorial code since they didn't need it

- You don't technically need to pick a homeworld in the beginning of the game.  They spent 4 hours just mining before they staked out homeworlds (though this didn't seem to give any advantage in the game)
...
They also made [a project for automatically tweeting out stats from the public ethereum smart contract](https://github.com/jacobrosenthal/sophon_tweets).  The Twitter account for the service was [@sophon_eth](https://twitter.com/sophon_eth).  This repo uses tokio for async and was discussed in the next section of the meetup.
...
In the Rust ecosystem, there are two major async runtimes, [async-std](https://async.rs/) and [tokio](https://tokio.rs/).  There is also the [smol](https://crates.io/crates/smol) async runtime ([which smol is now based on](https://github.com/async-rs/async-std/pull/757)) which is just the runtime without extra runtime-agnostic stuff (concurrency primitives, extension traits, etc.).
...
Smol's philosophy is: "Let's not keep reinventing API's, let's write one and just wrap synchronous things with it" (e.g. `smol::Async<<std::net::TcpSocket>>`).

**Why use Smol?** It can be split out into many crates.  If you don't need one piece, you can just swap it out.  For instance in embedded programming we can't use the smol executor because it uses std, so we can swap that out for a different executor.
