...
Nelson rants that there are too many wrappers for the Windows API that are all subtely different, none of which providing everything he needs.  The closest one is [winapi](https://crates.io/crates/winapi).
...
Repo: [https://github.com/mysteriouspants/uDevGames.com](https://github.com/mysteriouspants/uDevGames.com)
...
Chris rolled his own OAuth client instead of using the [oauth2](https://crates.io/crates/oauth2) crate.  GitHub's docs are so awesome that it is easy and fun to implement it yourself.
...
Chris played with using [rocket](http://rocket.rs/)'s async API.  In his [main()](https://github.com/mysteriouspants/uDevGames.com/blob/mainline/src/main.rs#L29), he uses the `rocket::main` macro, an alias for diesel is not async.  Then the [route handlers](https://github.com/mysteriouspants/uDevGames.com/blob/mainline/src/gh_oauth.rs) can use async await commands:
...
Jacob was iterating on it and did some benchmarking to compare his code's performance (e.g. how much time is being wasted on the scheduler, how much is rayon taking, etc.).  Performance was measured in hashes per second.  In addition to measuring results for different changes in the architecture, Jacob also measured results against different block_sizes for the hash function to see which block_size yielded the best results.  Another he analyzed performance was using [flamegraph](https://github.com/flamegraph-rs/flamegraph) to which function calls were taking the most time.

Chris mentioned that, for serious benchmarking, it is important to control the conditions under which the test is run.  He provided the following commands he uses to control his cpu-scaling and frequency, in order to stabilize benchmarks:
